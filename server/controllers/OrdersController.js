        ...
        if(
            ...
            if(customerInfoResp){
                ...
                productsInfo.map((value) => {
                    ...
                    let totalPrice = (parseInt(quantity) * parseFloat(value.product_price)).toFixed(2);
                ...
                if(ordersResp){
                    const replaceData = {
                        ...
                        pickupDate: reqBody.pickup_date,
                        pickupTimeSlot: pickupTimeData.from_time+ ' - ' +pickupTimeData.to_time,
                        dropDate: reqBody.drop_date,
                        dropTimeSlot: dropTimeData.from_time+ ' - ' +dropTimeData.to_time,
                        pincode: reqBody.pincode,
                        city: reqBody.city,
                        address1: reqBody.address1,
                        address2: reqBody.address2,
                    }
                    const sendMailResp = await sendMail(reqBody.email, replaceData, 'sendOrderConfirmation', 'sendinblue');
