...
export class ProductService {
  ...
  */
  ...
  private get products(): Observable<any> {
    this.Products = this.http.post<any>(`${this.apiURL}/web/getProducts`, {}).pipe(map((data: any) => data.products));
    // this.Products = this.http.get<any>('assets/web/data/products.json').pipe(map(data => data));
  ...
  public getProductBySlug(slug: string): any {
    // return [];
  }
  ...
  */
  ...
  // Get Compare Items
  public get compareItems(): Observable<any> {
    const itemsStream = new Observable(observer => {
  ...
  */
  ...
  public calculateStockCounts(product: { quantity: any; stock: any; }, quantity: number) {
  ...
  public cartTotalAmount(): Observable<number> {
    return this.cartItems.pipe(map((product: any) => {
      return product.reduce((prev: any, curr: any) => {
        let price = parseFloat(curr.price);
        if(curr.discount) {
          price = curr.price - (curr.price * curr.discount / 100)
        }
        return (parseFloat(prev) + price * parseInt(curr.quantity)).toFixed(2);
      }, 0);
    }));
  }
